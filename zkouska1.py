...
def process_numbers(numbers):
    result = [] #prázdný seznam 
    
    for num in numbers: # zkontroluje každý prvek v seznamu

        if isinstance(num, bool): # Kontrola, zda není prvek boolean - boolean = buď True nebo False
            break
        
        if not isinstance(num, (int, float)): # Kontrola, zda je prvek číslo (int (int je celé číslo) nebo float(float je desetinné číslo))
            break
        
        
        if num > 5: # Kontrola, zda je číslo větší než 5
            result.append(num * 2) #přidáme a vynásobíme *2
